...
int printAllLines(FileInfo *file_info) {
    if (file_info == NULL) {
        fprintf(stderr, "Null pointer error while printing file contents");
        return EXIT_FAILURE;
    }
    ...
    offset = lseek(file_info->file_descriptor, 0, SEEK_SET);
    if (offset == LSEEK_ERROR) {
        perror("Lseek error while printing file contents");
        return EXIT_FAILURE;
    }

    do {
        chars_read = read(file_info->file_descriptor, buff, BUFF_SIZE - 1);
        if (chars_read == READ_ERROR) {
            perror("Could not read from file to print file contents");
            return EXIT_FAILURE;
        }
        buff[chars_read] = '\0';
        printf("%s", buff);
...
int processInput(FileInfo *file_info) {
    ...
    do {
        ...
        if (error == TIME_EXCEEDED) {
            printf("Time exceeded, printing file contents:\n");
            return printAllLines(file_info);
        }
